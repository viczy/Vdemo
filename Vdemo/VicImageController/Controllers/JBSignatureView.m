...
- (id)initWithFrame:(CGRect)frame withImage:(UIImage *)image {
    ...
    if (self) {
        NSMutableArray *hwc = [[NSMutableArray alloc] init];
		self.handwritingCoords = hwc;
        [hwc release];
		self.lineWidth = 5.0f;
        ...
        [image drawInRect:CGRectMake(frame.origin.x, frame.origin.y, image.size.width, image.size.height)];
        if (nil != image) {
            bgImg_ = [image retain];
        }
    }
...
-(UIImage *)getSignatureImage {
    ...
    [bgImg_ drawInRect:CGRectMake(0.0, 0.0, self.frame.size.width, self.frame.size.height)];
